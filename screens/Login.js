    ...
    if (email !== "" && password !== "") {
      signInWithEmailAndPassword(auth, email, password)
        .then(async () => {
          ...
          const carSnapshot = await getDocs(collection(database, 'car'));
            ...
            if (doc.exists()) {
              ...
              if (userDoc.email === email) {
                ...
                carSnapshot.forEach((doc) => {
                  if (doc.exists()) {
                    const carDoc = doc.data();
                    if (carDoc.user_email === userEmail) {
                      carArray.push(carDoc);
                    }
                  }
                });
              }
          ...
          navigation.navigate('Home', { name, carArray });
        })
        .catch();
    }
  };
